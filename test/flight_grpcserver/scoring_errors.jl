...
@testset "cross-process Flight startup script surfaces native Julia DoExchange gRPC schema errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test occursin("requires columns: doc_id, vector_score", err.message)
        @test occursin("missing: doc_id, vector_score", err.message)
        ...
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight request batch requires columns: doc_id, vector_score; missing: doc_id, vector_score"
    end
end
...
@testset "cross-process Flight startup script surfaces native Julia DoExchange invalid score errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test err isa gRPCClient.gRPCServiceCallException
        @test err.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test occursin("column vector_score row 1 must contain numeric values", err.message)
        ...
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight request batch column vector_score row 1 must contain numeric values; got \"oops\"::String"
    end
end
...
@testset "cross-process Flight startup script surfaces native Julia DoExchange invalid doc_id errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test err isa gRPCClient.gRPCServiceCallException
        @test err.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test occursin("column doc_id row 1 must contain string values", err.message)
        @test occursin("42", err.message)
        @test occursin("::Int64", err.message)
        @test !occursin("ArgumentError", err.message)
        @test result.message_count == 0
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight request batch column doc_id row 1 must contain string values; got 42::Int64"
    end
end
...
@testset "cross-process Flight startup script surfaces native Julia DoExchange schema-version errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test err isa gRPCClient.gRPCServiceCallException
        @test err.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test occursin("requires schema version v1; got v999", err.message)
        @test !occursin("ArgumentError", err.message)
        @test result.message_count == 0
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "Arrow Flight exchange request requires schema version v1; got v999"
    end
end
...
@testset "cross-process Flight startup script surfaces native Julia DoExchange empty doc_id errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test err.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test occursin(
            "column doc_id row 1 must contain non-empty string values",
            err.message,
        )
        ...
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight request batch column doc_id row 1 must contain non-empty string values; got empty string"
    end
end
...
@testset "cross-process Flight startup script surfaces native Julia DoExchange duplicate doc_id errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight request batch column doc_id row 2 must contain unique non-empty string values; duplicate \"doc-a\" already seen at row 1"
    end
end
...
@testset "cross-process Flight startup script surfaces native Julia DoExchange nonfinite score errors" begin
    with_scoring_flight_server() do port, process
        ...
        @test occursin("got NaN::Float64", err.message)
        @test !occursin("ArgumentError", err.message)
        @test result.message_count == 0
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight request batch column vector_score row 1 must contain finite numeric values; got NaN::Float64"
    end
end
...
@testset "cross-process bad-response Flight startup script surfaces native Julia DoExchange scoring response errors" begin
    with_bad_response_flight_server() do port, process
        ...
        @test result.grpc_status == gRPCClient.GRPC_INVALID_ARGUMENT
        @test result.grpc_message ==
              "WendaoArrow stream scoring Flight response column analyzer_score row 1 must contain finite numeric values; got NaN::Float64"
    end
end
